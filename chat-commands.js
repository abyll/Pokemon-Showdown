...
exports.commands = {
	...
	as: function(target, room, user) {
		...
		if (room && room.id === 'lobby') return this.errorReply('Cannot set an alias in the lobby.');
		
		// Impersonation check
		let other = Users.get(alias);
		if (other) {
			if (other === user) alias = null;
			if (other !== user) return this.errorReply('You are not allowed to impersonate another user.');
			// Note: This doesn't handle offline people...
		}
		...
		if (msg) return `"${msg}"`;
		if (!alias) return this.sendReply(`Alias for ${where} has been unset.`);
		return this.sendReply(`Alias for ${where} set to ${alias}.`);
	},
	...
	avatar: function (target, room, user) {
		...
		if (!parts[1]) {
			this.sendReply("Avatar changed to:\n" +
				'|raw|<img src="//tppleague.me/sprites/trainers/' + (typeof avatar === 'string' ? avatar.substr(1) : avatar) + '.png" alt="" width="80" height="80" />');
		}
	},
	...
	join: function (target, room, user, connection) {
		...
		if (target.startsWith('tppleague.me/')) target = target.slice(13);
		if (user.tryJoinRoom(target, connection) === null) {
	...
	hotpatch: function (target, room, user) {
		...
		try {
			if (target === 'all') {
				...
				this.sendReply("Stadium requires uncached successfully.");
	...
	challenge: function (target, room, user, connection) {
		...
		user.prepBattle(Dex.getFormat(target, supBanlist), 'challenge', connection).then(validTeam => {
